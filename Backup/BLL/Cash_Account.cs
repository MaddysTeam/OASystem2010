...
namespace Dianda.BLL
{
	...
	{
		private readonly Dianda.DAL.Cash_Account dal=new Dianda.DAL.Cash_Account();
		public Cash_Account()
		{}
		#region  Method
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Dianda.Model.Cash_Account GetModelByCache(int ID)
		{
			
			string CacheKey = "Cash_AccountModel-" + ID;
			...
			return (Dianda.Model.Cash_Account)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Dianda.Model.Cash_Account> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Dianda.Model.Cash_Account> DataTableToList(DataTable dt)
		{
			List<Dianda.Model.Cash_Account> modelList = new List<Dianda.Model.Cash_Account>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Dianda.Model.Cash_Account model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Dianda.Model.Cash_Account();
					...
					if(dt.Rows[n]["FreezeTime"]!=null && dt.Rows[n]["FreezeTime"].ToString()!="")
					{
						model.FreezeTime=DateTime.Parse(dt.Rows[n]["FreezeTime"].ToString());
					}
                    ...
                    if (dt.Rows[n]["EveryDayNum"] != null && dt.Rows[n]["EveryDayNum"].ToString() != "")
                    {
                        model.EveryDayNum = decimal.Parse(dt.Rows[n]["EveryDayNum"].ToString());
                    }
					modelList.Add(model);
				}
			}
