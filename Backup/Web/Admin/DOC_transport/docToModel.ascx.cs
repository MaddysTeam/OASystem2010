...
{
    public partial class docToModel : System.Web.UI.UserControl
    {
   
        Dianda.COMMON.pageControl PageControl = new Dianda.COMMON.pageControl();// new COMMON.pageControl();//引入通用操作类
        BLL.DOC_transport_To DOC_transport_FromBll = new Dianda.BLL.DOC_transport_To();//引入发件箱的操作类
        ...
        private void setRowCout(string key, string KeySel)
        {
            try
            {
               ...
               // dt = DOC_transport_FromBll.GetList(sqlwhere1).Tables[0];
                int count = PageControl.tableRowCout(sqlwhere1, "DOC_transport_To", "ID");
        ...
        private string sqlWhere(string key, string KeySel)
        {
            ...
            if (KeySel != "")
            {
                coutw = coutw + " and (TITLES like '%" + KeySel + "%' or FROMUSER like '%" + KeySel + "%')";
            }
            coutw = coutw + " and TOUSER='" + Session["USERNAME_SYSUSER"].ToString() + "'";
            return coutw;
        }
        /// <summary>
        /// 分页显示
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow) //判断是否是DataRow，以防止鼠标经过Header也有效果
            {
                ...
                if (ISREADS == "0")
                {
                    cell5Text = "<font color=\"red\">未阅读</font>";
                }
                else
                {
                    cell5Text = "<a title='阅读时间：" + READTIMES + "'>已读</a>";
                }
                e.Row.Cells[5].Text = cell5Text;
                ...
                string dowhat = "<a href='check.aspx?ID=" + ID + "&Key=" + Key + "&KeySel=" + KeySel + "&pageindex=" + pageindex + "' target='_self'>-查看-</a>";
                e.Row.Cells[6].Text = dowhat+"<a href='delshou.aspx?ID=" + ID + "&Key=" + Key + "&KeySel=" + KeySel + "&pageindex=" + pageindex + "&names=" + TITLE + "' target='_self'>-删除-</a>";
            }
        }
