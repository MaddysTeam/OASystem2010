...
function tmpSelectYearInnerHTML(strYear) //年份的下拉框
{
    ...
    //for (var i = n; i < n + 101; i++) 徐巍注释 因为无法选择当前值以后的年份
    for (var i = n; i < new Date().getFullYear() + 5; i++)
    {
...
function checkTime(planTime,planTimeEnd,startTime,endTime)
{
    var obj;
    ...
    if(typeof(planTime) == "object"){
        obj = planTime;
        var plan_Time = planTime;
    }else{
        var plan_Time = document.getElementById(planTime);
    }
    ...
    if(typeof(planTimeEnd) == "object")
    {
        obj = planTimeEnd;
        var plan_TimeEnd = planTimeEnd;
    }else {
        var plan_TimeEnd = document.getElementById(planTimeEnd);
    }
    
    if(start_time.value!='' || plan_Time.value!='')
    {
        var now = new Date();
        if(new Date(start_time.value.replace(/-/g,"\/"))<now)
        {
            alert("报名时间应该不早于当前时间");
            start_time.value = "";
            start_time.focus();
        ...
        if(new Date(plan_Time.value.replace(/-/g,"\/"))<now)
        {
            alert("预计活动时间应不早于当前时间");
            plan_Time.value = "";
            plan_Time.focus();
            return false;
        }
    ...
    if(start_time.value!=''&&plan_Time.value!='')
    {
        if(!CompareDate(start_time.value,plan_Time.value))
        {
            alert('预计活动时间应晚于报名时间.');
            obj.value="";
            obj.focus();
    ...
    if(end_time.value!=''&&plan_Time.value!='')
    {
        if(!CompareDate(end_time.value,plan_Time.value))
        {
            alert('预计活动时间应晚于报名时间.');
            obj.value="";
            obj.focus();
    ...
    {
        if(!CompareDate(start_time.value,end_time.value))
        {
            alert('报名开始时间应早于报名结束时间');
            obj.value="";
            obj.focus();
            return false;
        }
    ...
    if(plan_Time.value!=''&&plan_TimeEnd.value!='')
    {
        if(!CompareDate(plan_Time.value,plan_TimeEnd.value))
        {
            alert('预计活动开始时间应该早于预计活动结束时间');
            obj.value="";
            obj.focus();
            return false;
        }
...
function checkString(controlId,labelContent,isEmpty)
{
    ...
    try
    {
    var strCheck = document.getElementById(controlId).value.Trim();
    var rgularExpression=/[~!@#\$%\^&\*\(\)\+=\{\}\[\];'<>/\?\|\\"]+/;
    var showMsg;
    if((strCheck=="")||(strCheck==null))
    {
        if(isEmpty==true)
        {
            showMsg = labelContent+"不能为空.";
            alert(showMsg);
            return false;
        }
    }
    ...
    {
        if(strCheck.search(rgularExpression)!=-1)
        {
        showMsg=labelContent+"包含非法字符.";
        alert(showMsg);
        return false;
        }
        return true;
